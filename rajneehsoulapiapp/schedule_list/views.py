...
#         elif hasattr(user, "userEmailLinked_id"):  # Custom user
#             return user, {"user_id": user.userEmailLinked_id}
#         else:
...
class ScheduleItemView(APIView):
    ...
    def get(self, request, item_id=None):
        ...
        if item_id:
            if "Bearer" not in request.headers["Authorization"]:
                user_scheduled_object = ScheduleItemList.objects.filter(user_id=user.userEmailLinked_id, id=item_id)
            else:
            ...
            if "Bearer" not in request.headers["Authorization"]:
                schedule_list_items = ScheduleItemList.objects.filter(user_id=user.userEmailLinked_id)
            else:
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            if "Bearer" not in request.headers["Authorization"]:
                serializer.save(user_id=user.userEmailLinked_id)
            else:
    ...
    def patch(self, request, item_id=None):
        ...
        schedule_item = get_object_or_404(ScheduleItemList, id=item_id, user_id=user.userEmailLinked_id)
    ...
    def delete(self, request, item_id = None):
        ...
        remaining_items = ScheduleItemList.objects.filter(user_id=user.userEmailLinked_id)
